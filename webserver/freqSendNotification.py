...
sentTokenColl = db["deviceTokens:sent"]
freqColl = db["frequency:received"]
...
while (1):
  time.sleep(2)
    if (list(freqColl.find().sort('_id',-1).limit(1))[0]["freq"] >= 50):
      tokens = tokenColl.find()
      if (tokens.count > 0):
        try:
          apns = APNs(use_sandbox=True, cert_file='ChrisElsmoreCert.pem', key_file='ChrisElsmoreKeyUn.pem')
            for tok in tokens:
              apns.gateway_server.send_notification(tok['token'], payload)
              tok['tsSent'] = int(time.time())
              sentTokenColl.insert(tok)
              tokenColl.remove({'_id':tok['_id']})
          print "Failed tokens:"
